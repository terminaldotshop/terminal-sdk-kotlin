...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String = asString() ?: throw TerminalInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw TerminalInvalidDataException("`$name` is not set")
            is JsonNull -> throw TerminalInvalidDataException("`$name` is null")
            else -> throw TerminalInvalidDataException("`$name` is invalid, received $this")
        }
